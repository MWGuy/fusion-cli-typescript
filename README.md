# fusion-cli-typescript

The CLI interface for Fusion.js with TypeScript support

The `fusion-cli-typescript` package is responsible for orchestrating compile-time configuration for server and browser bundles, as well as development, test and production variations. It provides a standardized Babel configuration that includes async/await support as well as stage 3+ Ecmascript features.
...
The CLI API can be most easily run through the Yarn or NPX CLI, e.g. `yarn fusion-ts build` or `npx fusion-ts build`.

- `fusion-ts build [dir] [--production] [--log-level] [--maxWorkers]`
  ...
  - `ENABLE_REACT_PROFILER=true fusion-ts build`

- `fusion-ts dev [dir] [--port] [--no-hmr] [--test] [--log-level] [--forceLegacyBuild]`
...
- `fusion-ts start [--environment]`
  Runs your application, assuming you have previously built them via `fusion-ts build`. Note that build artifacts must be saved to disk (i.e. this command will fail if you use `fusion-ts dev` to build artifacts instead of `fusion-ts build`.

  - `--environment`: Which environment/assets to run - defaults to first available assets among `["development", "production"]`

- `fusion-ts test [options]`
